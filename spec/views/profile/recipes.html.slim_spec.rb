...
RSpec.describe "profile/recipes", type: :view do
  ...
  context "when user hasn't created any recipes yet" do
    before do
      @pagy, @recipes = pagy_array(user.recipes.published, items: 12)
  ...
  context "when user has 5 non-draft recipes and pagy display 12 items per page" do
    before do
      10.times { |n| n % 2 == 0 ? create(:recipe) : create(:recipe, user: user) }
      create_list(:recipe, 3, user: user, status: :draft)
      @pagy, @recipes = pagy_array(user.recipes.published, items: 12)
      assign(:recipes, @recipes)
      assign(:pagy, @pagy)
      visit user_session_path
